...
runSEIR <- function (
  ...
  psi = 1, delta = 0, frac = 0,
  S0 = 500, E0 = 10, I0 = 10, R0 = 0
) {
  params <- c(Beta=Beta,sigma=sigma,gamma=gamma,psi=psi,delta=delta,frac=frac)
...
continueSEIR <- function (
  ...
  psi = NA, delta = NA, frac = NA
) {
  params <- c(
    Beta=Beta,sigma=sigma,gamma=gamma,psi=psi,delta=delta,frac=frac
  )
