// SEIR: Standard susceptible-exposed-infectious-recovered model (C++)
#include "master.h"
...
double seir_proc_t::event_rates (double *rate, int n) const {
  int m = 0;
  double total = 0;
  RATE_CALC(params.Beta * state.S * state.I / state.N);  // S -> E
  RATE_CALC(params.sigma * state.E);                     // E -> I 
  RATE_CALC(params.gamma * state.I);                     // I -> R
  RATE_CALC(params.psi * state.I);                       // sample I
  RATE_CALC(params.delta * state.R);                     // R -> S
...
template<>
void seir_genealogy_t::batch (void) {
