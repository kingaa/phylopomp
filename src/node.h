...
public:
  ...
  //! number of descendants (green or black balls not including own green)
...
public:
  ...
  //! Compact Newick format (with reassortment markers)
  std::string compact_newick (const slate_t& tnow, const slate_t& tpar) const {
    std::string o1 = "", o2 = "", o3 = "";
    int nc = nchildren();
    // determine label based on ball colors
    if (holds(blue)) {
      o3 = "b_";
    } else if (holds(darkorange)) {
      o3 = "#Ha1_";
    } else if (holds(lightorange)) {
      o3 = "#Ha2_";
    } else if (holds_own()) {
      o3 = "m_";
    } else {
    ...
    // only wrap in parens if node has children
    if (nc > 0) {
      o1 = "("; o3 = ")" + o3;
    }
    int n = 0;
