...
template<>
void twospecies_proc_t::update_params (double *p, int n) {
  int m = 0;
  PARAM_SET(Beta11);
  PARAM_SET(Beta12);
  PARAM_SET(Beta21);
...
template<>
void twospecies_proc_t::update_IVPs (double *p, int n) {
  int m = 0;
  PARAM_SET(S1_0);
  PARAM_SET(S2_0);
  PARAM_SET(I1_0);
...
template<>
double twospecies_proc_t::event_rates (double *rate, int n) const {
  int m = 0;
  double total = 0;
...
GENERICS(TwoSpecies,twospecies_genealogy_t)
