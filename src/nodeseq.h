...
public:
  ...
  //! drop the black ball 'a' and prune the tree as needed.
  //! Three cases based on pocket size and companion ball color:
  //! (1) size > 2: simply erase the black ball
  //! (2) size == 2: action depends on companion ball
  //! (3) size == 1: swap with green ball, destroy node, recurse
  void drop (ball_t *a) {
    if (!a->is(black))
      err("in '%s': ball %ld is not black (color=%d)",
          __func__, (long)a->uniq, (int)a->color);
    node_t *p = a->holder();
    if (p->size() > 2) {
      p->erase(a);
      delete a;
      ...
      ball_t *b = p->other(a);
      switch (b->color) {
      case darkorange: case lightorange:
        // orange marker node: swap black for green, destroy, recurse
        a->deme() = p->deme();
        swap(a,p->green_ball());
        destroy_node(p);
        drop(a);
        break;
      case blue:
        // sample node: just remove the black ball
        p->erase(a);
        delete a;
        break;
      case green:
        // green companion: swap other for green, destroy if same deme
        if (b->owner()->deme() == p->deme()) {
          swap(b,p->green_ball());
          destroy_node(p);
        } else {
          p->erase(a);
        ...
        break;
      case black:
        // another black ball: swap other for green, destroy if same deme
        if (b->deme() == p->deme()) {
          swap(b,p->green_ball());
          destroy_node(p);
        } else {
      ...
      // size == 1: only the black ball remains
      swap(a,p->green_ball());
      destroy_node(p);
      drop(a);
    }
  };
  //! remove a node that holds its own green ball.
  //! The node may hold up to one additional ball (e.g., a marker
  //! or a black ball being dropped), which gets deleted along with the node.
  void destroy_node (node_t *p) {
    if (!p->holds_own())
      err("destroy_node: node %ld does not hold its own green ball (size=%ld)",
          (long)p->uniq, (long)p->size());
    if (p->size() > 2)
      err("destroy_node: node %ld has size %ld > 2",
          (long)p->uniq, (long)p->size());
    remove(p);
    delete p;
  };
...
public:
  ...
  //! put genealogy at time `t` into standard Newick format.
  ...
  //! put genealogy at time `t` into compact Newick format
  //! (with reassortment markers and outer dummy wrapper).
  std::string compact_newick (slate_t t) const {
    slate_t te = dawn();
    std::string o = "(i_NA_NA:0.0,i_NA_NA:0.0";
    for (node_t *p : *this) {
      if (p->is_root()) {
        o += ",(" + p->compact_newick(t,te) + ")i_NA_NA:0.0";
      }
    }
    o += ")i_NA_NA;";
    return o;
